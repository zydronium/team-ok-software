...
{
    [TestClass]
    public class WorkspaceunitsTest
    {
        ...
        [TestMethod]
        public void GetStatusByMacAddressTest()
        {
            ...
            using (var context = new BackendDBContext(option))
            {
                ...
                var target = new WorkspaceunitsController(context);
                var result = target.GetStatusByMacAddress(workspace.MacAddress);
        ...
        [TestMethod]
        public void PostStatusByMacAddressTest()
        {
            var option = CreateNewContextOptions();
            using (var context = new BackendDBContext(option))
            {
                ...
                statusViewModel.Claimed = true;
                statusViewModel.ClaimedUntill = DateTime.Now.AddHours(1.0);

                var target = new WorkspaceunitsController(context);
                var result = target.PostStatusByMacAddress(workspace.MacAddress, statusViewModel);
                Assert.AreEqual(workspace.Claimed, statusViewModel.Claimed);
                Assert.AreEqual(workspace.ClaimedUntill, statusViewModel.ClaimedUntill);
            }
        }

        [TestMethod]
        public void PostStatusByMacAddressExpiredTimeTest()
        {
            var option = CreateNewContextOptions();
            using (var context = new BackendDBContext(option))
            {
                ...
                var target = new WorkspaceunitsController(context);
                var result = target.PostStatusByMacAddress(workspace.MacAddress, statusViewModel);
                Assert.AreNotEqual(workspace.Claimed, statusViewModel.Claimed);
            }
        }
    }
}
